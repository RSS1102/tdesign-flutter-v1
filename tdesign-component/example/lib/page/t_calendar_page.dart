      ...
      children: [
        Expanded(
          child: TCalendar(
            type: type,
            initialValue:
                seedInitial != null ? List<DateTime>.from(seedInitial!) : null,
            minDate: minDate,
            maxDate: maxDate,
...
class _AnchorCalendarCellState extends State<_AnchorCalendarCell> {
  List<DateTime> _selected =
      List<DateTime>.from(_AnchorDemoData.initialSelected);
...
class _AnchorPickerHint extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedLine =
        hasInitialValue ? '已预选 $selectedLabel' : '未预选日期，仅按锚点月份定位';
          ...
          if (days > 0)
            Container(
              ...
              child: Text(
                '共 $days 天',
                style:
                    TextStyle(fontSize: 12, color: context.tTheme.brandColor7),
              ),
            ),
        ],
      ),
...
class _RangeSegment extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(label,
            style: TextStyle(fontSize: 12, color: context.tTheme.fontGyColor3)),
...
class _StyleDemoState extends State<_StyleDemo> {
  ...
  Widget? _buildPriceSubtitle(
    ...
    final isSpecial = date.month == 2 && _specialDays.containsKey(date.day);
    final price = isSpecial ? '¥100' : '¥60';
    final label = isSpecial ? _specialDays[date.day] : null;
    final selected = subtitleContext.selectType == DateSelectType.selected;
    final theme = context.tTheme;
    final color =
        selected ? theme.fontWhColor1 : (isSpecial ? theme.errorColor6 : null);
                ...
                TCell(
                  ...
                  onTap: () {
                    _showCalendarPickerSheet(
                      ...
                      onConfirm: (value) => _customSubtitleSelected.value =
                          _normalizeDateList(value),
                    );
                  },
                ),
              ...
              Expanded(
                child: GridView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    ...
                    final isDisabled = monthKey < minKey || monthKey > maxKey;
                    final isSelected =
                    ...
                    return InkWell(
                      onTap:
                          isDisabled ? null : () => Navigator.pop(ctx, month),
                      child: Container(
                        alignment: Alignment.center,
                        decoration: BoxDecoration(
                          color: bgColor,
