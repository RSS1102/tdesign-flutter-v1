...
| value | bool | false | 选中状态。默认为`false` 当FuiCheckBox嵌入到FuiCheckBoxGroup的时候，这个值表示初始状态，后续的状态会由Group管理 |
...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| value | List<String>? | - | 勾选的CheckBox id列表 |
...
| onChanged | ValueChanged? | - | 状态变化监听器 |
...
```dart
typedef IconBuilder = Widget? Function(BuildContext context, bool value);
```
...
```dart
typedef ContentBuilder = Widget Function(BuildContext context, bool value, String? content);
```
