...
void checkStaticGrep() {
  ...
  for (final meta in componentList) {
    ...
    for (final f in listDartFiles(compDir)) {
      ...
      if (fileName.contains('theme.dart') || fileName.contains('theme_data.dart')) continue;
  ...
  for (final f in listDartFiles('$projectRoot/lib/src')) {
    ...
    if (f.path.endsWith('t_theme.dart')) continue;
  ...
  for (final meta in componentList) {
    ...
    for (final f in listDartFiles(compDir)) {
      ...
      if (fileName.contains('theme_data.dart') || fileName.contains('resolve.dart')) continue;
      // 排除非主 Widget 的辅助文件
      if (fileName.startsWith('t_') && !fileName.contains('_test')) {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (RegExp(r'Colors\.').hasMatch(line) && !colorWhitelist.hasMatch(line)) {
            // 排除 CupertinoColors
            if (line.contains('CupertinoColors.')) continue;
...
void checkExportConvergence() {
  ...
  final exportedFilesWithoutShow = <String>[];
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // 仅检查 _style.dart 文件中的 *Style（样式配置类），不检查 _theme_data.dart 中的 *Style 枚举
    if (fullLine.contains('show ') && fullLine.contains('_style.dart')) {
      ...
      for (final sym in symbols) {
        if (sym.endsWith('Style') && !sym.endsWith('ColorScheme')) {
...
void checkResolveSingleEntry() {
  ...
  for (final meta in resolveComponents) {
    ...
    if (mainWidgetFile.existsSync()) {
      ...
      for (var i = 0; i < lines.length; i++) {
        final line = lines[i];
        if (RegExp(r'Widget\s+build\s*\(').hasMatch(line)) inBuild = true;
        if (inBuild) {
...
void checkDisabledConvention() {
  ...
  for (final meta in componentList) {
    ...
    for (final f in listDartFiles(compDir)) {
      ...
      if (fileName != 't_${meta.dirName}.dart') continue;

      final content = readFile(f);
      final lines = content.split('\n');

      // 追踪当前是否在主 Widget 类内（class T{WidgetName} 到下一个 class 声明之间）
      bool inMainWidgetClass = false;
...
void checkDemoRegistration() {
  ...
  for (final meta in componentList) {
    if (meta.configKey == null) continue;
...
void checkCoverage() {
  ...
  for (final line in lines) {
    if (line.startsWith('SF:')) {
      ...
      if (current != null && parts.length >= 2) {
        ...
        current.totalLines++;
        if (hitCount > 0) current.coveredLines++;
      }
  ...
  for (final entry in fileCoverage.entries) {
    ...
    if (!path.contains('lib/src/components/')) continue;
...
void checkApiDocConsistency() {
  ...
  for (final meta in componentList) {
    if (meta.apiFolderName == null) continue;
    ...
    if (apiParams.isEmpty || docParams.isEmpty) continue;
...
Set<String> _extractConstructorParams(String content, String widgetName) {
  ...
  bool inConstructorSection = false;
  bool inTable = false;
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (!inConstructorSection) continue;
    ...
    if (inTable && line.startsWith('|')) {
      ...
      if (match != null) {
        ...
        // 排除表头关键词
        if (_isHeaderKeyword(name)) continue;
        // 排除全大写或含下划线的枚举值（如 DEFAULT_SIZE, primary 等）
        if (name == name.toUpperCase() && name.contains('_')) continue;
        params.add(name);
      }
    }
  }
...
/// 从生成 API md 中提取参数名（旧方法，保留备用）
Set<String> _extractParamNames(String content) {
  final params = <String>{};
  final lines = content.split('\n');
  for (final line in lines) {
    // 匹配 markdown 表格行 | 参数名 | 描述 | 或 | 属性名 |
    final match = RegExp(r'^\|\s*`?(\w+)`?\s*\|').firstMatch(line.trim());
    if (match != null) {
      final name = match.group(1)!;
      // 排除表头
      if (name == '参数名' || name == '属性名' || name == '参数' || name == '属性' ||
          name == '名称' || name == '类型' || name == '默认值' || name == '描述' ||
          name == '说明' || name == '---') continue;
      params.add(name);
    }
  }
  return params;
...
Set<String> _extractSection1Params(String content) {
  ...
  if (sectionStart < 0) return params;
  ...
  for (var i = sectionStart; i < sectionEnd; i++) {
    ...
    if (match != null) {
      ...
      if (name[0].toUpperCase() == name[0] && name.length > 1) continue;
      params.add(name);
    }
  }
  return params;
...
String generateReport() {
  ...
  buf.writeln('| **项G** Web 验收 | flutter build web | 见 CI |');
  buf.writeln();
...
String _mapResult(List<String> categories) {
  ...
  if (relevant.isEmpty) return '⏳ 未执行';
